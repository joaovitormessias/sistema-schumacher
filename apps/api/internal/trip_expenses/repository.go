...
func (r *Repository) List(ctx context.Context, filter ListFilter) ([]TripExpense, error) {
	query := `select id, trip_id, driver_id, expense_type, amount, description, expense_date, payment_method, driver_card_id, receipt_number, receipt_verified, receipt_verified_by, receipt_verified_at, receipt_verification_notes, is_approved, approved_by, approved_at, notes, created_by, created_at, updated_at from trip_expenses`
	args := []interface{}{}
	clauses := []string{}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.TripID, &item.DriverID, &item.ExpenseType, &item.Amount, &item.Description, &item.ExpenseDate, &item.PaymentMethod, &item.DriverCardID, &item.ReceiptNumber, &item.ReceiptVerified, &item.ReceiptVerifiedBy, &item.ReceiptVerifiedAt, &item.ReceiptVerificationNotes, &item.IsApproved, &item.ApprovedBy, &item.ApprovedAt, &item.Notes, &item.CreatedBy, &item.CreatedAt, &item.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *Repository) Get(ctx context.Context, id uuid.UUID) (TripExpense, error) {
	var item TripExpense
	row := r.pool.QueryRow(ctx, `select id, trip_id, driver_id, expense_type, amount, description, expense_date, payment_method, driver_card_id, receipt_number, receipt_verified, receipt_verified_by, receipt_verified_at, receipt_verification_notes, is_approved, approved_by, approved_at, notes, created_by, created_at, updated_at from trip_expenses where id=$1`, id)
	if err := row.Scan(&item.ID, &item.TripID, &item.DriverID, &item.ExpenseType, &item.Amount, &item.Description, &item.ExpenseDate, &item.PaymentMethod, &item.DriverCardID, &item.ReceiptNumber, &item.ReceiptVerified, &item.ReceiptVerifiedBy, &item.ReceiptVerifiedAt, &item.ReceiptVerificationNotes, &item.IsApproved, &item.ApprovedBy, &item.ApprovedAt, &item.Notes, &item.CreatedBy, &item.CreatedAt, &item.UpdatedAt); err != nil {
		return item, err
	}
	return item, nil
}
...
func (r *Repository) Create(ctx context.Context, input CreateTripExpenseInput, createdBy *string, performedBy *string) (TripExpense, error) {
	...
	row := tx.QueryRow(ctx,
     ...
     returning id, trip_id, driver_id, expense_type, amount, description, expense_date, payment_method, driver_card_id, receipt_number, receipt_verified, receipt_verified_by, receipt_verified_at, receipt_verification_notes, is_approved, approved_by, approved_at, notes, created_by, created_at, updated_at`,
...
func (r *Repository) Update(ctx context.Context, id uuid.UUID, input UpdateTripExpenseInput) (TripExpense, error) {
	...
	args = append(args, id)
	query := fmt.Sprintf(`update trip_expenses set %s where id=$%d returning id, trip_id, driver_id, expense_type, amount, description, expense_date, payment_method, driver_card_id, receipt_number, receipt_verified, receipt_verified_by, receipt_verified_at, receipt_verification_notes, is_approved, approved_by, approved_at, notes, created_by, created_at, updated_at`, strings.Join(sets, ", "), idx)
